...
app.post('/make-server-a611b057/auth/signup', async (c) => {
  try {
    ...
    const orgName = organizationName && organizationName.trim() ? organizationName : fullName + "'s Organization";
    
    const organization = {
      id: organizationId,
      name: orgName,
      shortName: orgName.split(' ').map(w => w[0]).join('').toUpperCase().slice(0, 3),
...
app.post('/make-server-a611b057/organizations', async (c) => {
  try {
    ...
    return c.json({ error: `Server error creating organization: ${error}` }, 500);
  }
});
...
app.get('/make-server-a611b057/organizations', async (c) => {
  try {
    ...
    if (userOrgs.length > 0) {
      console.log('📊 Organization details:', userOrgs.map(o => ({
        id: o.id,
        name: o.name,
    ...
    return c.json({ error: `Server error getting organizations: ${error}` }, 500);
  }
});
...
app.put('/make-server-a611b057/organizations/:id', async (c) => {
  try {
    ...
    return c.json({ error: `Server error updating organization: ${error}` }, 500);
  }
});
...
app.post('/make-server-a611b057/programs', async (c) => {
  try {
    ...
    if (!organizationId || !program) {
      return c.json({ error: 'Organization ID and program data are required' }, 400);
    }
    ...
    if (organization.ownerId !== user.id) {
      return c.json({ error: 'Unauthorized to add programs to this organization' }, 403);
    }
...
// Update program
app.put('/make-server-a611b057/programs/:organizationId/:programId', async (c) => {
  try {
    const { user, error } = await verifyUser(c.req.header('Authorization'));
    if (error) {
      return c.json({ error }, 401);
    ...
    if (organization.ownerId !== user.id) {
      return c.json({ error: 'Unauthorized to update programs in this organization' }, 403);
    }

    const programIndex = organization.programs.findIndex(p => p.id === programId);
...
app.post('/make-server-a611b057/certificates', async (c) => {
  try {
    ...
    const { organizationId, programId, certificateHeader, courseName, courseDescription, completionDate, template, students, customTemplateConfig } = requestBody;
    ...
    if (isNewFormat) {
      ...
      if (!organizationId || !programId || !students || !Array.isArray(students)) {
        console.log('❌ Missing required fields for old format');
        return c.json({ error: 'Organization ID, program ID, and students array are required' }, 400);
      }
    }
    ...
    if (!organization) {
      ...
      return c.json({ error: `Organization not found: ${organizationId}` }, 404);
    }
    ...
    if (organization.ownerId !== user.id) {
      ...
      return c.json({ error: 'Unauthorized: You do not own this organization' }, 403);
    }
    ...
    if (template) {
      ...
      if (templateData && templateData.type === 'premium') {
        ...
        const isPremiumOrg = organization.tier === 'premium' || organization.subscriptionStatus === 'active';
        
        if (!isPremiumOrg) {
          console.log('❌ Organization does not have premium access');
          return c.json({ 
            error: 'This template requires a premium subscription. Please upgrade to use premium templates.',
    ...
    if (isNewFormat) {
      ...
      console.log('✅ Certificate saved to KV store with key: cert:' + certificateId);
      certificates.push(certificate);

      // Update program certificate count if program exists
      if (programId) {
        console.log('📊 Attempting to update program statistics for programId:', programId);
        ...
        console.log('ℹ️ No programId provided, skipping program statistics update');
      }
    } else {
    ...
    console.log('✅ Successfully generated', certificates.length, 'certificate(s)');
    return c.json({ certificates, count: certificates.length });
  } catch (error) {
    console.log('❌ Error generating certificates:', error);
    return c.json({ error: `Server error generating certificates: ${error}` }, 500);
  }
});
  ...
  try {
    ...
    if (organization) {
      ...
      if (settings) {
        ...
        console.log('📜 Organization settings loaded:', settings.signatories?.length || 0, 'signatories');
      }
    }
    
    const program = organization?.programs.find(p => p.id === certificate.programId);
    console.log('📜 Program found:', program ? 'YES (ID: ' + program.id + ')' : 'NO (searching for: ' + certificate.programId + ')');
...
app.get('/make-server-a611b057/organizations/:id/certificates', async (c) => {
  try {
    ...
    if (organization.ownerId !== user.id) {
      console.log('❌ User not authorized to view certificates for this organization');
      return c.json({ error: 'Unauthorized to view certificates for this organization' }, 403);
    }
    ...
    if (orgCerts.length > 0) {
      console.log('📜 Certificate IDs:', orgCerts.map(c => c.id));
    }

    return c.json({ certificates: orgCerts });
  } catch (error) {
    console.log('❌ Error getting organization certificates:', error);
    return c.json({ error: `Server error getting certificates: ${error}` }, 500);
  }
});
...
app.delete('/make-server-a611b057/certificates/:id', async (c) => {
  try {
    ...
    return c.json({ error: `Server error deleting certificate: ${error}` }, 500);
  }
});
...
app.delete('/make-server-a611b057/certificates', async (c) => {
  try {
    ...
    console.log('🗑️ Bulk delete request for', certificateIds.length, 'certificates');
    ...
    console.log(`✅ Deleted ${deletedCount} certificate(s), ${errors.length} error(s)`);
    ...
    return c.json({ error: `Server error deleting certificates: ${error}` }, 500);
  }
});
...
app.post('/make-server-a611b057/certificates/:id/testimonial', async (c) => {
  try {
    const certificateId = c.req.param('id');
    const { studentName, testimonial, courseName, organizationId, programId } = await c.req.json();
    ...
    if (!studentName || !testimonial || !courseName || !organizationId) {
      return c.json({ error: 'Student name, testimonial, course name, and organization ID are required' }, 400);
    }
    ...
    const testimonialId = `TEST-${Date.now()}-${Math.random().toString(36).substr(2, 9).toUpperCase()}`;
    ...
    await kv.set(`org_testimonial:${organizationId}:${testimonialId}`, testimonialData);
    ...
    return c.json({ error: `Server error submitting testimonial: ${error}` }, 500);
  }
});
...
app.get('/make-server-a611b057/organizations/:id/testimonials', async (c) => {
  try {
    ...
    if (organization.ownerId !== user.id) {
      console.log('❌ User not authorized to view testimonials for this organization');
      return c.json({ error: 'Unauthorized to view testimonials for this organization' }, 403);
    }

    // Get all testimonials for this organization
    console.log('📊 Searching for testimonials with prefix: org_testimonial:' + organizationId);
    const testimonials = await kv.getByPrefix(`org_testimonial:${organizationId}`);
    ...
    return c.json({ error: `Server error fetching testimonials: ${error}` }, 500);
  }
});
...
app.delete('/make-server-a611b057/programs/:orgId/:progId', async (c) => {
  try {
    ...
    if (organization.ownerId !== user.id) {
      console.log('❌ User not authorized to delete programs in this organization');
      return c.json({ error: 'Unauthorized to delete programs in this organization' }, 403);
    }
...
app.post('/make-server-a611b057/upload', async (c) => {
  try {
    ...
    if (!file || !type || !organizationId) {
      return c.json({ error: 'File, type, and organizationId are required' }, 400);
    }
    ...
    const organization = await kv.get(`org:${organizationId}`);
    if (!organization || organization.ownerId !== user.id) {
      return c.json({ error: 'Unauthorized to upload files for this organization' }, 403);
    }
    ...
    const fileName = `${organizationId}/${type}-${Date.now()}.${file.name?.split('.').pop() || 'png'}`;
...
app.get('/make-server-a611b057/organizations/:id/settings', async (c) => {
  try {
    ...
    if (organization.ownerId !== user.id) {
      return c.json({ error: 'Unauthorized to view settings for this organization' }, 403);
    }
...
app.put('/make-server-a611b057/organizations/:id/settings', async (c) => {
  try {
    ...
    if (organization.ownerId !== user.id) {
      return c.json({ error: 'Unauthorized to update settings for this organization' }, 403);
    }
    ...
    organization.primaryColor = settings.primaryColor || organization.primaryColor;
...
app.post('/make-server-a611b057/testimonials', async (c) => {
  try {
    const { certificateId, studentName, email, rating, text, isPublic } = await c.req.json();

    if (!certificateId || !studentName || !rating || !text) {
      return c.json({ error: 'Certificate ID, student name, rating, and text are required' }, 400);
    }
    ...
    return c.json({ error: `Server error creating testimonial: ${error}` }, 500);
  }
});
...
app.get('/make-server-a611b057/organizations/:id/testimonials', async (c) => {
  try {
    ...
    const orgTestimonials = allTestimonials.filter(test => test.organizationId === organizationId);

    return c.json({ testimonials: orgTestimonials });
  } catch (error) {
    console.log('Error getting testimonials:', error);
    return c.json({ error: `Server error getting testimonials: ${error}` }, 500);
  }
});
...
app.get('/make-server-a611b057/organizations/:id/analytics', async (c) => {
  try {
    ...
    if (organization.ownerId !== user.id) {
      console.log('❌ User not authorized to view analytics for this organization');
      return c.json({ error: 'Unauthorized to view analytics for this organization' }, 403);
    }

    // Get all certificates for this organization
    const allCertificates = await kv.getByPrefix(`org_certificate:${organizationId}`);
    ...
    const engagementRate = totalCertificates > 0 ? Math.round((totalTestimonials / totalCertificates) * 100) : 0;
    ...
    const coursePerformance = Object.keys(certificatesByCourse).map(courseName => ({
      name: courseName,
    ...
    for (let i = 5; i >= 0; i--) {
      ...
      const monthEnd = new Date(year, date.getMonth() + 1, 0, 23, 59, 59).getTime();
...
app.get('/make-server-a611b057/templates', async (c) => {
  try {
    ...
    const freeTemplates = allTemplates.filter(t => 
      t.id === 'template1' || t.id === 'template2' || t.id === 'template3' || 
      t.id === 'template4' || t.id === 'template5'
    );
    const premiumTemplates = allTemplates.filter(t => t.value?.type === 'premium');
    const combinedTemplates = [...freeTemplates, ...premiumTemplates];
    
    console.log('✅ Found', combinedTemplates.length, 'template(s) in global library');
    console.log('   📗 Free:', freeTemplates.length, '|', '👑 Premium:', premiumTemplates.length);
    console.log('📋 Template IDs:', combinedTemplates.map(t => t.id).join(', '));
...
app.post('/make-server-a611b057/templates/seed', async (c) => {
  try {
    ...
    if (existing.length > 0) {
      console.log('⚠️ Templates already exist, skipping seed. Use /templates/force-reseed to reset.');
      return c.json({ message: 'Templates already seeded', count: existing.length });
    }
    ...
    const defaultTemplates = [
      {
        ...
        description: 'Classic design with brown/gold border, decorative corners, and elegant award badge',
        config: {
          colors: {
            background: '#faf8f3',
            border: '#8b6f47',
      ...
      },
      {
        ...
        description: 'Classic ornamental design with decorative floral borders, elegant swirls, and traditional styling',
        config: {
          colors: {
            background: '#faf8f3',
            border: '#4a3728',
      ...
      },
      {
        ...
        description: 'Modern professional design with navy blue waves, gold accents, and decorative badge',
        config: {
          colors: {
            background: '#ffffff',
            border: '#1e3a8a',
      ...
      },
      {
        ...
        description: 'Bold modern design with red diagonal chevrons, ribbon badge, and dynamic styling',
        config: {
          colors: {
            background: '#ffffff',
            border: '#dc2626',
      ...
      },
      {
        ...
        description: 'Elegant design with green diagonal stripes, decorative scalloped badge, and cream background',
        config: {
          colors: {
            background: '#f5f5dc',
            border: '#1b5e20',
...
app.post('/make-server-a611b057/templates/force-reseed', async (c) => {
  try {
    ...
    console.log('✅ Force reseeded', defaultTemplates.length, 'default template(s) (Templates 1-5)');
  ...
  try {
    ...
    const defaultTemplates = [
      ...
      // ===== PREMIUM TEMPLATES START HERE =====
      {
        ...
        description: 'PREMIUM: Elegant certificate with gold medallion seal, brown/orange accents, and social media icons',
        config: {
          colors: {
            background: '#ffffff',
            border: '#ea580c',
    ...
    const freeCount = defaultTemplates.filter(t => t.type === 'default').length;
    const premiumCount = defaultTemplates.filter(t => t.type === 'premium').length;

    console.log('✅ Force reseeded', defaultTemplates.length, 'template(s):', freeCount, 'free +', premiumCount, 'premium');
    ...
    return c.json({ error: `Server error force reseeding templates: ${error}` }, 500);
  }
});
...
app.post('/make-server-a611b057/templates/purge-and-reset', async (c) => {
  try {
    ...
    return c.json({ 
      message: 'All old templates purged. Only Template 1 (Certificate of Appreciation) remains.', 
      template: template1 
    });
  } catch (error) {
...
app.get('/make-server-a611b057/billing/config', async (c) => {
  try {
    const settings = await kv.get('billing:settings');
...
app.post('/make-server-a611b057/billing/initialize', async (c) => {
  try {
    ...
    if (!settings || !settings.paystackSecretKey) {
      ...
      return c.json({ 
        error: 'Billing is not configured yet. Please contact support to enable premium features.',
        requiresSetup: true 
      }, 503);
    }
    ...
    const response = await fetch('https://api.paystack.co/transaction/initialize', {
      ...
      body: JSON.stringify({
        ...
        callback_url: `${c.req.header('origin') || 'https://example.com'}/#/dashboard?payment_status=success`,
        metadata: {
          planId,
          userId: user.id,
          organizationId,
...
app.post('/make-server-a611b057/billing/verify', async (c) => {
  try {
    ...
    if (data.data.status !== 'success') {
      return c.json({ error: 'Payment not successful', status: data.data.status }, 400);
    }
...
app.post('/make-server-a611b057/billing/webhook', async (c) => {
  try {
    ...
    // Get billing settings for webhook validation
    const settings = await kv.get('billing:settings');
    if (!settings) {
      return c.json({ error: 'Billing not configured' }, 503);
    }
...
const logBillingActivity = async (organizationId: string, activity: {
  type: 'payment_initiated' | 'payment_success' | 'payment_failed' | 'subscription_activated' | 'subscription_cancelled' | 'subscription_expired' | 'subscription_renewed';
  description: string;
  metadata?: any;
}) => {
  ...
  console.log(`📝 Billing activity logged: ${activity.type} for org ${organizationId}`);
};

// Get billing activities for an organization
app.get('/make-server-a611b057/billing/activities/:organizationId', async (c) => {
  try {
    const { user, error } = await verifyUser(c.req.header('Authorization'));
    if (error) {
      return c.json({ error }, 401);
    ...
    const allActivities = await kv.getByPrefix(`activity:org:${organizationId}:`);
...
app.get('/make-server-a611b057/billing/subscription/:organizationId', async (c) => {
  try {
    const { user, error } = await verifyUser(c.req.header('Authorization'));
    if (error) {
      return c.json({ error }, 401);
...
app.get('/make-server-a611b057/billing/transactions/:organizationId', async (c) => {
  try {
    const { user, error } = await verifyUser(c.req.header('Authorization'));
    if (error) {
      return c.json({ error }, 401);
...
app.post('/make-server-a611b057/billing/subscription/:organizationId/cancel', async (c) => {
  try {
    const { user, error } = await verifyUser(c.req.header('Authorization'));
    if (error) {
      return c.json({ error }, 401);
...
app.get('/make-server-a611b057/admin/stats', async (c) => {
  try {
    ...
    const premiumOrgs = organizations.filter(org => org && org.plan === 'premium').length;
    const freeOrgs = organizations.filter(org => org && (org.plan === 'free' || !org.plan)).length;
...
app.get('/make-server-a611b057/admin/organizations', async (c) => {
  try {
    ...
    for (const item of allOrgs) {
      ...
      console.log(`✅ Added organization: ${orgData.name} (${orgData.id}) - Owner: ${ownerEmail}`);
...
app.get('/make-server-a611b057/admin/templates', async (c) => {
  try {
    ...
    const allTemplates = await kv.getByPrefix('globaltemplate:');
    const templates = allTemplates
      .filter(item => item.value && typeof item.value === 'object' && item.value.id)
      .map(item => ({
        id: item.value.id,
...
app.post('/make-server-a611b057/admin/organizations/:organizationId/membership', async (c) => {
  try {
    ...
    console.log(`👑 Admin granting premium to org ${organizationId} for ${durationMonths} months`);
    ...
    const activityLogKey = `billing_activity_log:${Date.now()}:${Math.random().toString(36).substring(7)}`;
    const activityEntry = {
    ...
    console.log(`✅ Premium granted to ${org.name} until ${expiryDate.toISOString()}`);
    ...
    return c.json({ 
      error: `Failed to grant membership: ${error instanceof Error ? error.message : String(error)}`,
...
app.delete('/make-server-a611b057/admin/organizations/:organizationId/membership', async (c) => {
  try {
    ...
    console.error('❌ Error stack:', error instanceof Error ? error.stack : 'No stack trace');
    return c.json({ 
      error: `Failed to revoke membership: ${error instanceof Error ? error.message : String(error)}`,
      details: error instanceof Error ? error.stack : undefined
    }, 500);
  }
});
...
app.get('/make-server-a611b057/admin/debug-keys', async (c) => {
  try {
    ...
    const prefixes = ['org:', 'user:', 'cert:', 'globaltemplate:', 'customtemplate:', 'payment:', 'program:', 'testimonial:'];
    ...
    const summary = {
      totalKeys: Object.values(keysByPrefix).reduce((sum, items) => sum + items.length, 0),
      byPrefix: Object.fromEntries(
        Object.entries(keysByPrefix).map(([prefix, items]) => [prefix, items.length])
      ),
    };
...
app.get('/make-server-a611b057/admin/organizations/paid', async (c) => {
  try {
    ...
    for (const item of allOrgs) {
      ...
      const orgTransactions = allTransactions
        .map(t => t.value)
        .filter(tx => tx && tx.organizationId === org.id && tx.status === 'success')
        .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
      ...
      if (subscription || orgTransactions.length > 0) {
        ...
        paidOrganizations.push({
          ...
          createdAt: org.createdAt || new Date().toISOString(),
          subscription: subscription || null,
          totalPaid: orgTransactions.reduce((sum, tx) => sum + (tx.amount || 0), 0) / 100, // Convert from kobo to naira
...
app.get('/make-server-a611b057/admin/users-organizations', async (c) => {
  try {
    ...
    if (allUsers.length > 0) {
      console.log('📝 Sample user keys:', allUsers.slice(0, 3).map(item => item.key));
    }
    ...
    for (const item of allUsers) {
      ...
      if (user.organizationId) {
        ...
        const userTransactions = allTransactions
          .map(t => t.value)
          .filter(tx => tx && tx.organizationId === user.organizationId && tx.status === 'success')
          .sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
        
        totalPaid = userTransactions.reduce((sum, tx) => sum + (tx.amount || 0), 0) / 100;
      ...
      usersOrganizations.push({
        ...
        organizationName: organization?.name || user.organizationName || 'No Organization',
    ...
    return c.json({ error: `Failed to get users/organizations: ${error}` }, 500);
  }
});
...
app.post('/make-server-a611b057/admin/organizations/:organizationId/membership', async (c) => {
  try {
    ...
    console.log(`👑 Admin granting membership to org ${organizationId}: ${planName} for ${durationMonths} months`);
...
app.put('/make-server-a611b057/admin/organizations/:organizationId/subscription', async (c) => {
  try {
    const authHeader = c.req.header('Authorization');
...
app.post('/make-server-a611b057/admin/users/grant-premium', async (c) => {
  try {
    ...
    if (!organizationId || !durationDays) {
      return c.json({ error: 'Organization ID and duration are required' }, 400);
    }

    console.log(`🎁 Admin: Granting premium access to org ${organizationId} for ${durationDays} days...`);
...
app.post('/make-server-a611b057/admin/users/extend-premium', async (c) => {
  try {
    ...
    if (!organizationId || !additionalDays) {
      return c.json({ error: 'Organization ID and additional days are required' }, 400);
    }

    console.log(`⏰ Admin: Extending premium access for org ${organizationId} by ${additionalDays} days...`);
...
app.post('/make-server-a611b057/admin/users/revoke-premium', async (c) => {
  try {
    ...
    console.log(`🚫 Admin: Revoking premium access for org ${organizationId} with ${gracePeriodDays} day grace period...`);
...
app.get('/make-server-a611b057/custom-templates/template/:templateId', async (c) => {
  try {
    const { user, error } = await verifyUser(c.req.header('Authorization'));
    if (error) {
      return c.json({ error }, 401);
...
app.post('/make-server-a611b057/custom-templates', async (c) => {
  try {
    ...
    if (!organizationId || !template) {
      return c.json({ error: 'Organization ID and template data are required' }, 400);
    }
    ...
    // Save to KV store
    await kv.set(`customtemplate:${organizationId}:${templateId}`, customTemplate);
...
app.put('/make-server-a611b057/custom-templates/:templateId', async (c) => {
  try {
    ...
    // Find the template
    const allTemplates = await kv.getByPrefix('customtemplate:');
    const templateItem = allTemplates.find(item => 
      item.value && 
      typeof item.value === 'object' && 
      item.value.id === templateId
    );
    ...
    // Save to KV store
    await kv.set(`customtemplate:${existingTemplate.organizationId}:${templateId}`, updatedTemplate);
...
app.delete('/make-server-a611b057/custom-templates/:templateId', async (c) => {
  try {
    ...
    await kv.del(`customtemplate:${existingTemplate.organizationId}:${templateId}`);
...
app.post('/make-server-a611b057/certificates', async (c) => {
  try {
    ...
    if (!organizationId || !certificate) {
      return c.json({ error: 'Organization ID and certificate data are required' }, 400);
    }

    // Create the certificate object with metadata
    const savedCertificate = {
      id: certificate.id || `cert_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
      organizationId,
      studentName: certificate.studentName,
      email: certificate.email,
      programName: certificate.programName || certificate.program?.name,
      programDescription: certificate.programDescription || certificate.program?.description,
    ...
    await kv.set(`certificate:${organizationId}:${savedCertificate.id}`, savedCertificate);

    console.log(`✅ Certificate saved: ${savedCertificate.id} for org ${organizationId}`);
...
app.post('/make-server-a611b057/certificates/bulk', async (c) => {
  try {
    ...
    if (!organizationId || !certificates || !Array.isArray(certificates)) {
      return c.json({ error: 'Organization ID and certificates array are required' }, 400);
    }
    ...
    for (const cert of certificates) {
      const savedCertificate = {
        id: cert.id || `cert_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`,
        organizationId,
        studentName: cert.studentName,
        email: cert.email,
        programName: cert.programName || cert.program?.name,
        programDescription: cert.programDescription || cert.program?.description,
    ...
    console.log(`✅ Bulk save: ${savedCertificates.length} certificates for org ${organizationId}`);

    return c.json({ certificates: savedCertificates, count: savedCertificates.length });
  } catch (error) {
...
app.get('/make-server-a611b057/certificates/:organizationId', async (c) => {
  try {
    ...
    // Get all certificates for this organization
    const allCertificates = await kv.getByPrefix(`certificate:${organizationId}:`);
    ...
    console.log(`📜 Retrieved ${certificates.length} certificates for org ${organizationId}`);
...
app.get('/make-server-a611b057/certificates/cert/:certificateId', async (c) => {
  try {
    ...
    const certificateItem = allCertificates.find(item => 
      item.value && 
      typeof item.value === 'object' && 
...
app.delete('/make-server-a611b057/certificates/:certificateId', async (c) => {
  try {
    ...
    // Delete from KV store
    await kv.del(`certificate:${existingCertificate.organizationId}:${certificateId}`);
...
app.post('/make-server-a611b057/admin/seed-templates', async (c) => {
  try {
    ...
    return c.json({ 
      ...
      templates: defaultTemplates.map(t => ({ id: t.id, name: t.name, type: t.type }))
    });
  } catch (error) {
...
app.get('/make-server-a611b057/admin/platform-data', async (c) => {
  try {
    ...
    const enrichedOrgs = await Promise.all(validOrgs.map(async (org) => {
      // Find the owner user
...
app.get('/make-server-a611b057/billing/config', async (c) => {
  try {
    ...
    // Get billing settings from KV store
    const billingSettings = await kv.get('billing:settings');
    
    if (!billingSettings || !billingSettings.paystackSecretKey || !billingSettings.paystackPublicKey) {
...
app.get('/make-server-a611b057/billing/transactions/:organizationId', async (c) => {
  try {
    ...
    const allTransactions = await kv.getByPrefix(`transaction:${organizationId}:`);
...
app.post('/make-server-a611b057/billing/initialize', async (c) => {
  try {
    ...
    const { organizationId, planId } = await c.req.json();
    console.log(`💳 Initializing payment for org: ${organizationId}, plan: ${planId}`);
    ...
    const paystackResponse = await fetch('https://api.paystack.co/transaction/initialize', {
      ...
      body: JSON.stringify({
        ...
        reference,
        callback_url: `${c.req.header('origin') || 'https://localhost'}/#/dashboard?tab=billing&reference=${reference}`,
        metadata: {
          organizationId,
          organizationName: organization.name,
          planId,
...
app.post('/make-server-a611b057/billing/verify', async (c) => {
  try {
    ...
    const transaction = await kv.get(`transaction:${organizationId}:${reference}`);
    if (!transaction) {
...
app.post('/make-server-a611b057/billing/webhook', async (c) => {
  try {
    ...
    if (event === 'charge.success') {
      ...
      // Get billing settings for plan details
      const billingSettings = await kv.get('billing:settings');
      if (!billingSettings) {
      ...
      console.log(`✅ Webhook: Subscription activated for org ${organizationId}`);
    }
...
app.get('/make-server-a611b057/admin/billing/settings', async (c) => {
  try {
      ...
      return c.json({
        ...
        plans: {
          premium_monthly: { 
            ...
            currency: 'USD',
            duration: 30,
            features: ['Custom Templates', 'Template Builder', 'Unlimited Certificates', 'Priority Support']
          },
          premium_yearly: { 
            name: 'Premium Yearly',
            price: 10000,
            currency: 'USD',
            duration: 365,
            features: ['Custom Templates', 'Template Builder', 'Unlimited Certificates', 'Priority Support', '2 Months Free']
          }
        }
      });
    }
    ...
    return c.json({
      ...
      paystackSecretKeyPreview: settings.paystackSecretKey ? `${settings.paystackSecretKey.substring(0, 10)}...` : '',
      plans: settings.plans,
