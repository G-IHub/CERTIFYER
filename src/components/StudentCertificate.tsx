...
import { decryptCertificateData, getCertificateLinkTimeRemaining } from '../utils/encryption';
...
const StudentCertificate: React.FC<StudentCertificateProps> = ({ subsidiaries }) => {
  ...
  console.log('📍 URL Parameters:', { subsidiaryId, programId, certificateId, wildcardParam });
  ...
  useEffect(() => {
    const fetchCertificate = async () => {
      ...
      console.log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
      console.log('🔍 Current URL:', window.location.href);
      console.log('🔍 URL params:', { subsidiaryId, programId, certificateId, wildcardParam });
      ...
      if (wildcardParam && !subsidiaryId && !programId) {
        ...
        console.log('   - Already URL-decoded by React Router?', isAlreadyDecoded);
        
        // Pass the parameter as-is if already decoded, or pass it encoded
        const paramToDecrypt = isAlreadyDecoded ? encodeURIComponent(wildcardParam) : wildcardParam;
        ...
        if (decryptedData) {
          ...
          console.error('❌ Failed to decrypt certificate URL - link may be invalid or expired');
        ...
        console.log('⚠️ URL params:', { subsidiaryId, programId, certificateId, wildcardParam });
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        if (response.certificate) {
          ...
          console.log('   - Student Name:', cert.studentName || '(none - will prompt)');
          ...
          if (cert.customTemplateConfig) {
            console.log('🎨 Certificate has customTemplateConfig - using saved design');
            console.log('🎨 Custom config keys:', Object.keys(cert.customTemplateConfig));
            console.log('⚠️ NOT loading template from global library (would overwrite)');
            ...
            console.log('📋 No customTemplateConfig - loading template from backend:', cert.template);
            try {
              const templateResponse = await templateApi.getById(cert.template);
              if (templateResponse.template) {
                setTemplateConfig(templateResponse.template.config);
                console.log('✅ Template config loaded from global library:', templateResponse.template.name);
              }
            } catch (error) {
          ...
          if (!cert.studentName) {
            ...
            console.log('📄 Will display certificate directly (no name entry needed)');
          }
        } else {
          console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
          ...
          toast.error('Certificate not found - this certificate may not exist in the database');
        }
      } catch (error: any) {
        console.error('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
        ...
        if (error.message.includes('not found')) {
          errorMessage = 'Certificate not found. It may not have been saved to the database.';
    ...
    try {
      ...
      canvas.toBlob((blob) => {
        if (blob) {
          ...
          const courseName = certificate?.courseName || certificate?.program?.name || 'Certificate';
          const fileName = `${courseName.replace(/\s+/g, '_')}_${displayName.replace(/\s+/g, '_')}.png`;
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      // Download PDF
      const courseName = certificate?.courseName || certificate?.program?.name || 'Certificate';
      const fileName = `${courseName.replace(/\s+/g, '_')}_${displayName.replace(/\s+/g, '_')}.pdf`;
  ...
  const handleShare = (platform: string) => {
    ...
    const courseName = certificate?.courseName || certificate?.program?.name || 'Course';
  ...
  if (showNameForm) {
    ...
    const courseName = certificate.courseName || certificate.program?.name || 'this course';
    ...
    const handleFormSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        toast.success(enteredTestimonial.trim() 
          ...
          <CardContent className="p-8">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600">
                For <span className="font-semibold">{courseName}</span> from {orgName}
              </p>
            </div>
              ...
              <Card className="overflow-hidden">
                <CardContent className="p-0">
                  <div ref={certificateRef} className="w-full">
                    <CertificateRenderer
                      templateId={certificate.template || progData?.template || 'template1'}
                      header={certificate.certificateHeader || 'Certificate of Completion'}
                      courseTitle={certificate.courseName || progData?.name || 'Course'}
                      description={certificate.courseDescription || progData?.description || ''}
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3 text-sm">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">{orgData?.shortName || orgData?.name || 'N/A'}</span>
                    </div>
                    <div className="flex items-center gap-2">
                  ...
                  {showFullDetails && (
                    <div className="mt-4 pt-4 border-t space-y-3 text-sm">
                      <div>
                        ...
                        <p className="mt-1 text-gray-800">{certificate.courseDescription || progData?.description || 'N/A'}</p>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="font-medium">{certificate.downloadCount || 0} times</p>
                      </div>
                    </div>
                  )}
                </CardContent>
              ...
              <Card className="border-green-200 bg-green-50">
                <CardContent className="p-6">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <h3 className="font-bold text-green-800">Verified Certificate</h3>
                  </div>
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div className="text-center">
              <p className="text-gray-600 mb-2">
                © 2024 {orgData?.name || 'Certificate Platform'}. All rights reserved.
              </p>
              <p className="text-sm text-gray-500">
                This digital certificate is powered by a secure verification platform
              </p>
            </div>
          </div>
        </footer>
